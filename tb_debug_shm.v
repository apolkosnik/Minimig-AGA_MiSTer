...
// Debug shared memory write detection
module tb_debug_shm;
    ...
    always @(posedge clk) begin
        if (sel_ethernet) begin
            $display("DEBUG: cpu_addr=0x%06x, is_reg=%b, is_data_port=%b, cpu_wr=%b", 
                     {cpu_addr, 1'b0}, uut.is_register_access, uut.is_data_port_access, uut.cpu_wr);
            $display("       cpu_addr[9:6]=%b, shm_write_state=%d", cpu_addr[9:6], uut.shm_write_state);
        end
    ...
    // Test sequence
    initial begin
        $display("Starting Shared Memory Write Debug");
        ...
        $display("\n=== Test Data Port Write Address Detection ===");
        @(negedge clk);
