...
entity tb_jmp_record37_cpustore is
end entity;

architecture behavior of tb_jmp_record37_cpustore is
...
begin
    ...
    test: process
        ...
        begin
            ...
            -- High-memory dest seed kept as array write (not the failing case).
            write_word(16#4204FEFE#, x"CD54");

            -- Establish the low-memory prestate via CPU STORES (as cputest does),
            -- NOT testbench injection. This forces the core's own write path to
            -- make $8A/$8C/$8E visible to the later ORI reads across a JMP.
            write_word(pc, x"31FC"); pc := pc + 2; -- MOVE.W #$4AFC,($008A).W  -> $8B=FC
            write_word(pc, x"4AFC"); pc := pc + 2;
            write_word(pc, x"008A"); pc := pc + 2;
            write_word(pc, x"31FC"); pc := pc + 2; -- MOVE.W #$2048,($008C).W  -> $8D=48
            write_word(pc, x"2048"); pc := pc + 2;
            write_word(pc, x"008C"); pc := pc + 2;
            write_word(pc, x"31FC"); pc := pc + 2; -- MOVE.W #$4B16,($008E).W
            write_word(pc, x"4B16"); pc := pc + 2;
            write_word(pc, x"008E"); pc := pc + 2;
            ...
            -- JMP to the ORI tail (separates the prestate store from the read,
            -- mirroring cputest setup -> JMP -> ORI on the real hardware).
            write_word(pc, x"4EF9"); pc := pc + 2; -- JMP ($42001100).L
            write_long(pc, x"42001100"); pc := pc + 4;
