-- tb_group2_t0_trace.vhd
-- Settled MC68030 Group 2 trace tests for legal T0 trace mode cases.
-- Uses an explicit NOP after MOVE to SR so T0 is armed before the tested
-- instruction, then checks the stacked trace frame layout for real Group 2
-- instruction traps.
...
entity tb_group2_t0_trace is
end entity;

architecture behavior of tb_group2_t0_trace is
...
begin
    ...
    begin
        ...
        report "Group 2 T0 Trace Regression" severity note;
        report "MC68030 UM 8.1.7 / 8.1.12 settled T0 cases" severity note;
        report "========================================" severity note;

        report "" severity note;
        report "TEST 1: CHK.W with T0=1 (stacked trace)" severity note;
        init_memory;
        ...
        mem(16#1002# / 2) := x"FFFF";
        mem(16#1004# / 2) := x"323C";
        mem(16#1006# / 2) := x"000A";
        mem(16#1008# / 2) := x"46FC";
        mem(16#100A# / 2) := x"6000";
        mem(16#100C# / 2) := x"4E71";
        mem(16#100E# / 2) := x"4181";
        mem(16#1010# / 2) := x"4E71";
        clear_stack;
        do_reset;
        wait_for_stop;
        ...
        check_pc("T1: CHK.W PC=$1010", x"00001010");
        check_ia("T1: CHK.W IA=$100E", x"0000100E");

        report "" severity note;
        report "TEST 2: CHK.L with T0=1 (stacked trace)" severity note;
        init_memory;
        setup_vector(16#18#, 16#2000#);
        setup_vector(16#24#, 16#2100#);
        setup_handler(16#2000#);
        ...
        mem(16#1006# / 2) := x"323C";
        mem(16#1008# / 2) := x"000A";
        mem(16#100A# / 2) := x"46FC";
        mem(16#100C# / 2) := x"6000";
        ...
        check_pc("T2: CHK.L PC=$1012", x"00001012");
        check_ia("T2: CHK.L IA=$1010", x"00001010");

        report "" severity note;
        report "TEST 3: CHK2.B with T0=1 (stacked trace)" severity note;
        init_memory;
        setup_vector(16#18#, 16#2000#);
        setup_vector(16#24#, 16#2100#);
        setup_handler(16#2000#);
        ...
        mem(16#100A# / 2) := x"00FF";
        mem(16#100C# / 2) := x"46FC";
        mem(16#100E# / 2) := x"6000";
        mem(16#1010# / 2) := x"4E71";
        mem(16#1012# / 2) := x"00D0";
        ...
        check_pc("T3: CHK2.B PC=$1016", x"00001016");
        check_ia("T3: CHK2.B IA=$1012", x"00001012");

        report "" severity note;
        report "TEST 4: TRAP #5 with T0=1 (stacked trace)" severity note;
        init_memory;
        ...
        check_pc("T4: TRAP#5 PC=$1008", x"00001008");

        report "" severity note;
        report "TEST 5: TRAPV with T0=1, V=1 (stacked trace)" severity note;
        init_memory;
        ...
        setup_handler(16#2100#);
        mem(16#1000# / 2) := x"46FC";
        mem(16#1002# / 2) := x"6002";
        mem(16#1004# / 2) := x"4E71";
        mem(16#1006# / 2) := x"4E76";
        mem(16#1008# / 2) := x"4E71";
        clear_stack;
        do_reset;
        wait_for_stop;
        ...
        check_pc("T5: TRAPV PC=$1008", x"00001008");
        check_ia("T5: TRAPV IA=$1006", x"00001006");

        report "" severity note;
        report "TEST 6: DIVS.W #0,D0 with T0=1 (stacked trace)" severity note;
        init_memory;
        ...
        check_pc("T6: DIV0 PC=$1010", x"00001010");
        check_ia("T6: DIV0 IA=$100C", x"0000100C");

        report "" severity note;
        report "TEST 7: TRAPLT.W with T0=1 (stacked trace)" severity note;
        init_memory;
        setup_vector(16#1C#, 16#2300#);
        setup_vector(16#24#, 16#2100#);
        setup_handler(16#2300#);
        ...
        check_pc("T7: TRAPcc PC=$100C", x"0000100C");
        check_ia("T7: TRAPcc IA=$1008", x"00001008");
