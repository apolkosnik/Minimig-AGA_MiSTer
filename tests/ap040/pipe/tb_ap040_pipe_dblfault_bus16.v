...
// frame), the machine halted, and an interrupt does not wake it.           //
//--------------------------------------------------------------------------//
...
task run_case;
	...
	begin
		...
		vec_berr = 1'b0; lo = 32'h1000; hi = 32'h1000; want_w = 0;
		case (c)
		...
		default: begin   // C: address error, vector 3 odd
		...
		$display("ce %0s case %0s: %0s (%0d write sub-cycles)", cemode ? "random" : "on", (c == 0) ? "A" : (c == 1) ? "B" : "C",
