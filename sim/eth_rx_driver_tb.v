...
//
// Uses the REAL ethernet_interface + REAL eth_ddr3_mailbox + a behavioral
...
module eth_rx_driver_tb;
    ...
    initial begin
        ...
        reg_write(R_DCR,    8'h49);          // DCR = word mode (CardType 2 uses 32-bit port)
        ...
        //      from shm 0x2000 and transmits). This is the on-wire byte order;
        //      the HW symptom is every 16-bit word byte-swapped. ----
        repeat (4000) @(posedge clk_sys);   // let the bg stage all 64 bytes to shm
        begin : tx_order
            integer b; reg [7:0] sb; integer swaperr;
            swaperr = 0;
            for (b = 0; b < 64; b = b + 1) begin
                sb = ddr_read_byte(16'h2000 + b[15:0]);
                if (sb !== frame[b]) begin
                    errors = errors + 1;
                    if (sb === frame[b ^ 1]) swaperr = swaperr + 1;
                    if (b < 16)
                        $display("FAIL: TX shm[0x%04x]=0x%02x, frame[%0d]=0x%02x%s",
                                 16'h2000+b, sb, b, frame[b],
