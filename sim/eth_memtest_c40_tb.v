// ===========================================================================
// eth_memtest_c40_tb.v
//
// Reproduces the X-Surf TestPrg "Testing 16bit memory" step against the REAL
// ethernet_interface + REAL eth_ddr3_mailbox + a behavioral f2sdram slave, WITH
...
module eth_memtest_c40_tb;
    ...
    ethernet_interface dut (
        ...
        .cpu_rd(cpu_rd), .cpu_hwr(cpu_hwr), .cpu_lwr(cpu_lwr),
        ...
        .eth_irq(eth_irq), .dtack_eth(dtack_eth)
    );

    eth_ddr3_mailbox mailbox (
    ...
    task automatic data_port_write_word;
        ...
        begin
            ...
            @(negedge clk_sys);
            cpu_addr = 15'h0620; cpu_data_in = value;   // X-Surf 16-bit data port at 0xEA0C40 (the xsurftest port, reg 0x10)
    ...
    task automatic data_port_read_word;
        ...
        begin
            ...
            @(negedge clk_sys);
            cpu_addr = 15'h0620; sel_ethernet = 1'b1; cpu_rd = 1'b1;   // 16-bit data port 0xEA0C80
            cpu_as = 1'b0; cpu_uds = 1'b0; cpu_lds = 1'b0;
            for (w = 0; w < 600 && !done; w = w + 1) begin
                @(posedge clk_sys); #1;
    ...
    initial begin
        ...
        write_high_reg(15'h061c, 8'h01);   // DCR = 0x01 (word mode)
        ...
        if (errors == 0)
            $display("PASS: eth_memtest_c40_tb completed (remote-DMA mem round-trip OK with bg active)");
        else
            $display("eth_memtest_c40_tb FAILED with %0d error(s)", errors);
        $finish;
    end
