...
- Current disposition from those benches:
  ...
  - exact sparse-memory correction: the first version of that bench was still underwired. It did not map the real opcode window at `$42050000`, so the DUT silently fetched fallback `NOP`s, and its trace stub never unwound its own `BSR` frame before `RTE`, so any case that needed the trace handler to return into a pending Group 2/exception handler could not complete. Both detector bugs are now fixed in-tree.
  - exact sparse-memory result: with the opcode window and trace return path corrected, the exact BASIC reproducer now catches the real remaining failures much more cleanly:
  - `CHK2.B/0001 record=0 group=2 subcase=0`: stacked trace now hits, but its stacked PC is late by one instruction word (`$000018C6` vs expected `$000018C0`), the pending exception-6 handler never runs before the cycle budget expires, and after the trace return the core falls into an illegal stream (`opc=$A0D6`) instead of the vector-6 handler body
  - `CHK2.L/0001 record=0 group=2 subcase=0`: same failure shape as `CHK2.B`; stacked trace is taken with `PC=$000018C6`, but the exception-6 handler never runs and the core falls into the same wrong post-trace stream
  - `CHK2.W/0001 record=0 group=4 subcase=0`: now matches the packed BASIC case cleanly; standalone trace SR is `$8000` and stacked PC is `$42050004`
  - `JMP/0002 record=37 group=3 subcase=0`: standalone trace now hits, but the trace frame returns to the wrong PC (`$42006000` vs expected `$42006D72`), so exception 11 never runs and the packaged side effects still do not occur (`$008B` stays `$00`, `$008C` stays `$0000`, `$4204FEFF` stays `$54`)
