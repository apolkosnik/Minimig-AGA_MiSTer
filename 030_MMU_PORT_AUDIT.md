...
- Current disposition from those benches:
  ...
  - exact sparse-memory result: the earlier “passes in isolation” BASIC micro-benches were too synthetic. The exact bench now reproduces the real hardware `basic/all` failures directly:
  - `CHK2.B/0001 record=0 group=2 subcase=0`: no stacked trace handler entry and no exception-6 handler entry before the cycle budget expires
  - `CHK2.L/0001 record=0 group=2 subcase=0`: no stacked trace handler entry and no exception-6 handler entry before the cycle budget expires
  - `CHK2.W/0001 record=0 group=4 subcase=0`: standalone trace SR matches cputest (`$8000`), but stacked PC is early by one word (`$42050002` vs expected `$42050004`)
  - `JMP/0002 record=37 group=3 subcase=0`: no standalone trace handler entry, no exception-11 handler entry, and none of the packaged side effects occur (`$008B` stays `$00`, `$008C` stays `$0000`, `$4204FEFF` stays `$54`)
