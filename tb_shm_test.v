...
// Simple test to verify shared memory writes are working
module tb_shm_test;
    ...
    // Test sequence
    initial begin
        $display("Starting Shared Memory Write Test");
        ...
        $display("\n=== Test CR Register Write (should trigger shared memory write) ===");
        @(negedge clk);
        cpu_addr = {ethernet_base, 8'h0C, 4'h0, 2'b00}; // 0xEA0C00 - CR register
        cpu_data_in = 16'h1200;  // CR = 0x12 (Remote Write + Start)
        sel_ethernet = 1;
        cpu_rd = 1;
        cpu_hwr = 0;
        cpu_lwr = 0;
        ...
        $display("\n=== Test Data Port Write (should trigger shared memory write) ===");
        @(negedge clk);
