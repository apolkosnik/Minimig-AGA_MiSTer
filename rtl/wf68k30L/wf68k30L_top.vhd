...
--   Removed Exception handler data for stack format x"9" (coprocessor stack frame).
--   Exception stack frame format #2 has now a correct instruction address (INSTR_ADR).
--   Control: modifications in the MOVEM_CONTROL section to remove compiler detected combinatorial loops.
--   Control: modification of the OP_SIZE_I multiplexer (PACK, UNPK) to remove compiler detected combinatorial loops.
--
...
begin
    ...
    -- This register holds a copy of the program
    -- counter when the exception handler becomes
    -- active. 
    begin
        wait until CLK = '1' and CLK' event;
        if BUSY_EXH = '0' then
    ...
    end process INSTRUCTION_ADDRESS;
    ...
    DATA_EXH <= -- Exception handler multiplexing:
                ...
                INSTR_ADR when STACK_FORMAT = x"2" and STACK_POS = 6 else
