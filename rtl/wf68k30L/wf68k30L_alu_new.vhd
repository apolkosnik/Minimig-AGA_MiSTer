...
--   Generally used X_IN instead of STATUS_REG(4).
-- 
...
begin
    ...
    COND_CODES: process(BF_DATA_IN, BF_LOWER_BND, BF_UPPER_BND, BF_WIDTH, BIW_1, BITPOS, CB_BCD, CHK2CMP2_DR, CHK_CMP_COND, CLK, OP1, OP1_SIGNEXT, 
                        ...
                        RESULT_SHIFTOP, RESULT_OTHERS, SHIFT_WIDTH, STATUS_REG, VFLAG_DIV, XFLAG_SHFT, X_IN)
    ...
    begin
        ...
        case OP is
            ...
            when AND_B | ANDI | EOR | EORI | OR_B | ORI | NOT_B =>
                XNZVC <= X_IN & RESULT_LOGOP(MSB) & Z & "00";
            ...
            when BFCHG | BFCLR | BFEXTS | BFEXTU | BFFFO | BFSET | BFTST =>
                XNZVC <= X_IN & BF_DATA_IN(BF_UPPER_BND) & Z & "00";
            when BFINS =>
                XNZVC <= X_IN & OP1(BF_WIDTH -1) & Z & "00";
            when CLR =>
                XNZVC <= X_IN & "0100";
            ...
            when DIVS | DIVU =>
                XNZVC <= X_IN & NFLAG_DIV & Z & VFLAG_DIV & '0';
            when EXT | EXTB | MOVE | TST =>
                XNZVC <= X_IN & RESULT_OTHERS(MSB) & Z & "00";
            when MOVEQ =>
                if OP1_SIGNEXT(7 downto 0) = x"00" then
                    XNZVC <= X_IN & "0100";
                else
                    XNZVC <= X_IN & OP1_SIGNEXT(7) & "000";
                end if;
            when MULS | MULU =>
                XNZVC <= X_IN & NFLAG_MUL & Z & VFLAG_MUL & '0';
            when NEG | NEGX =>
                XNZVC(4) <= DM or RM;
                --
                if Z = '1' then
            ...
            when others => -- TAS, Byte only.
                XNZVC <= X_IN & OP2_SIGNEXT(MSB) & Z & "00";
        end case;
    ...
    -- Status register conditions: (X_IN = X, STATUS_REG(3) = N, STATUS_REG(2) = Z, STATUS_REG(1) = V, STATUS_REG(0) = C.)
