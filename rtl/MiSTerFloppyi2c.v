//////////////////////////////////////////////////////////////////////////////
// Largely based on the i2c code already within the MiSTer Minimig core

module MiSTerFloppyi2c
(
	input        CLK,
...
assign I2C_SCL = (SCLK | I2C_CLOCK) ? 1'b1 : 1'b0;
assign I2C_enableRead = SDO[3] ? 1'b1 : 1'b0;
...
always @(posedge CLK) begin
	...
	if (i_reset) begin
		...
		if(~old_st && START) begin
			...
			END  <= 0;
			rd   <= READ;
			len  <= I2C_WLEN;             // R/W   ACK   DATA         // force NAK on READ - this signals END of transfer
			...
			if(~old_clk && I2C_CLOCK && ~&SD_COUNTER) begin
				...
				case(SD_COUNTER)
						...
						20: if(rd)  SCLK <= 1;
						21: if(rd)  END  <= 1;   // Was 23
						28: if(~rd) ACK  <= ACK | I2C_SDA;
						29: if(~rd) SCLK <= 1;
						30: if(~rd) END  <= 1;   // was 32
				endcase
