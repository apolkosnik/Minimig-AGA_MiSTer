...
begin
	...
	validity_check: process(decode_enable, opcode, coprocessor_id, format_field, opmode_field)
		variable v_valid_f_line : std_logic;
		variable v_valid_coprocessor_id : std_logic;
		variable v_valid_format : std_logic;
		variable v_valid_opmode : std_logic;
	begin
		-- Check F-line prefix
		...
		if coprocessor_id = "001" then
			v_valid_coprocessor_id := '1';
		else
			v_valid_coprocessor_id := '0';
		end if;
		...
		if (opmode_field >= "0000000" and opmode_field <= "0001111") or  -- Basic arithmetic (0x00-0x0F)
		   (opmode_field >= "0100000" and opmode_field <= "0101111") or  -- Arithmetic group (0x20-0x2F)
		   (opmode_field >= "0110000" and opmode_field <= "0111111") or  -- WinUAE table includes FCMP/FTST at 0x38/0x3A
		...
		valid_f_line <= v_valid_f_line;
		valid_coprocessor_id <= v_valid_coprocessor_id;
		valid_format <= v_valid_format;
		valid_opmode <= v_valid_opmode;
		
		-- Overall validity (enhanced with comprehensive validation)
		valid_instruction <= decode_enable and v_valid_f_line and v_valid_coprocessor_id and v_valid_format and v_valid_opmode;
		illegal_instruction <= decode_enable and not (v_valid_f_line and v_valid_coprocessor_id and v_valid_format and v_valid_opmode);
